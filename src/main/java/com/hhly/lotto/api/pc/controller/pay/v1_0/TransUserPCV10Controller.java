 ...
 */
@RestController
@RequestMapping("/pc/v1.0/trans")
public class TransUserPCV10Controller extends BaseController {

	private static final Logger logger = Logger.getLogger(TransUserPCV10Controller.class);
	...
	*/
	...
	public Object transList(HttpServletRequest request) {
		...
		try {
			return transUserService.findTransUserByPage(transParam);
		} catch (Exception e) {
	...
	*/
	...
	public Object rechargeList(HttpServletRequest request) {
		...
		TransParamVO transParam = (TransParamVO) resultBO.getData();
		try {
			return transRechargeService.findRechargeListByPage(transParam);
		} catch (Exception e) {
			logger.error(ResultBO.getMsg(MessageCodeConstants.HESSIAN_ERROR_SYS, e.getMessage()));
			return ResultBO.getMsg(MessageCodeConstants.HESSIAN_ERROR_SYS, "TransUserPCV10Controller.userTransList");
		}
	...
	*/
	...
	public Object takenList(HttpServletRequest request) {
		...
		TransParamVO transParam = (TransParamVO) resultBO.getData();
		try {
			transParam.setPlatform(PayConstants.TakenPlatformEnum.WEB.getKey());
	...
	*/
	...
	public Object transRedList(HttpServletRequest request) {
		...
		try {
			...
			logger.error(ResultBO.getMsg(MessageCodeConstants.HESSIAN_ERROR_SYS, e.getMessage()));
			return ResultBO.getMsg(MessageCodeConstants.HESSIAN_ERROR_SYS, "TransUserPCV10Controller.transRedList");
		}
	}
