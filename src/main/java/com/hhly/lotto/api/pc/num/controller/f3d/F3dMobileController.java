...
/**
 * @desc 福彩3d手机端抽象类（定义安卓、ios、h5端共有信息）
 * @author huangb
 * @date 2017年10月21日
 * @company 益彩网络
 * @version v1.0
 */
public abstract class F3dMobileController extends F3dController {

	private static Logger logger = Logger.getLogger(F3dMobileController.class);

	/**
	 * @desc 移动端接口：福彩3d：查询彩种、当彩期、最新开奖详情、注/倍数配置、子玩法、限号
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：福彩3d：查询彩种、当彩期、最新开奖详情、注/倍数配置、子玩法、限号
	 * @throws Exception 
	 */
	@RequestMapping(value = "/info", method = RequestMethod.GET)
	@ResponseBody
	public Object findF3d() throws Exception {
		...
		List<F3dOmitOutBO> trendDigits = f3dTrendService.findOmitChanceColdHotAll(lot).getData();
	...
	/**
	 * @desc 移动端接口：福彩3d：查询最近开奖详情列表
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：福彩3d：查询最近开奖详情列表
	 */
	...
	public Object findRecentDrawDetail() {
		...
		return f3dTrendService.findRecentTrendSimple(new LotteryVO(getLottery(), NUMConstants.NUM_10));
	}
	
	/**
	 * @desc 移动端接口：福彩3d：查询遗漏/冷热（统计期数内出现最多或最少的号码）/概率（出号概率=号码当前遗漏值/号码历史平均遗漏值）数据
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：福彩3d：查询遗漏/冷热（统计期数内出现最多或最少的号码）/概率（出号概率=号码当前遗漏值/号码历史平均遗漏值）数据
	 * @throws Exception 
	 */
	...
	public Object findOmitChanceColdHot(LotteryVO param) throws Exception {
		...
		return f3dTrendService.findOmitChanceColdHotAll(param);
	}
	
	/**
	 * @desc 移动端接口：福彩3D：查询走势投注列表
	 * @author huangb
	 * @date 2017年3月6日
	 * @param param(qryCount期数)
	 * @return 移动端接口：福彩3D：查询走势投注列表
	 */
	...
	public Object findTrendBettingInfo(LotteryVO param) {
		...
		param.setLotteryCode(getLottery());
		return f3dTrendService.findRecentTrend(param);
	}
}
