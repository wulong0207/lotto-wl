...
/**
 * @desc 大乐透手机端抽象类（定义安卓、ios、h5端共有信息）
 * @author huangb
 * @date 2017年10月21日
 * @company 益彩网络
 * @version v1.0
 */
public abstract class DltMobileV10Controller extends DltV10Controller {
	
	/**
	 * @desc 移动端接口：大乐透：查询彩种、当彩期、最新开奖详情、注/倍数配置、子玩法、限号
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：大乐透：查询彩种、当彩期、最新开奖详情、注/倍数配置、子玩法、限号
	 * @throws Exception 
	 */
	@RequestMapping(value = "/info", method = RequestMethod.GET)
	public Object findDlt(HttpServletRequest request) throws Exception {
		...
		Assert.isTrue(!(null == lotBase && null == trend),MessageCodeConstants.DATA_NOT_EXIST);

		return ResultBO.ok(new NumBO(lotBase, trend));
	}
	
	/**
	 * @desc 移动端接口：大乐透：查询最近开奖详情列表
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：大乐透：查询最近开奖详情列表
	 */
	...
	/**
	 * @desc 移动端接口：大乐透：查询遗漏/冷热（统计期数内出现最多或最少的号码）/概率（出号概率=号码当前遗漏值/号码历史平均遗漏值）
	 *       数据
	 * @author huangb
	 * @date 2017年3月6日
	 * @return 移动端接口：大乐透：查询遗漏/冷热（统计期数内出现最多或最少的号码）/概率（出号概率=号码当前遗漏值/号码历史平均遗漏值
	 *         ）数据
	 * @throws Exception
	 */
	@RequestMapping(value = "/omit", method = RequestMethod.GET)
	public Object findOmitChanceColdHot(LotteryVO param) throws Exception {
		logger.debug("{}接口：{}:查询遗漏/冷热数据",getPlatform().getDesc(),getLotName());
