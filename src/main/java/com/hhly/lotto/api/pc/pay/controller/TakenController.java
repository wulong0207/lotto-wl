package com.hhly.lotto.api.pc.pay.controller;
...
@RestController
@RequestMapping("/pc/taken")
public class TakenController extends BaseController {

	private static final Logger logger = Logger.getLogger(TakenController.class);
	...
	*/
	...
	public Object takenReq(String token) {
		try {
			ResultBO<?> resultBO = transTakenService.takenReq(token);
			if (resultBO.isError()) {
	...
	* @auth: xiongJinGang
	* @param request
	* @time: 2017年4月20日 下午4:40:13
	* @return: Object 
	*/
	@RequestMapping(value = "/takenValidateReq", method = RequestMethod.GET)
	public Object takenValidateReq(HttpServletRequest request) {
		ResultBO<?> resultBO = PayCommon.validateTakenParams(request);
		if (resultBO.isError()) {
			logger.info("验证提款参数不通过：" + resultBO.getMessage());
			return resultBO;
		}
		TakenValidateTypeVO takenValidateType = (TakenValidateTypeVO) resultBO.getData();
		try {
			resultBO = transTakenService.takenReqValidate(takenValidateType);
			if (resultBO.isError()) {
				logger.info("申请提款失败：" + resultBO.getMessage());
			}
			return resultBO;
		} catch (Exception e) {
			logger.error("提款前验证基本信息异常，请求参数【" + takenValidateType.toString() + "】", e);
			return ResultBO.getMsg(MessageCodeConstants.HESSIAN_ERROR_SYS, "transTakenService.takenReqValidate");
		}
	}
	...
	*/
	...
	public Object takenConfirm(@RequestBody TakenInputReqParamVO TakenInputReqParamVO) {
		...
		if (resultBO.isError()) {
			logger.info("验证确认提款参数失败：" + resultBO.getMessage());
			return resultBO;
		}
		TakenReqParamVO takenReqParamVO = (TakenReqParamVO) resultBO.getData();
