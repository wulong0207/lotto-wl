package com.hhly.lotto.api.android.home.controller;
 ...
 */
@RestController
@RequestMapping("/android/home")
public class HomeAndroidController extends BaseController{
	private static final Logger LOGGER = Logger.getLogger(HomeAndroidController.class);
	@Autowired
	...
	public Object findMobialHome(OperateVO vo,HttpServletRequest request) {
		...
		try {
			operArtList=iOperateArticleService.findMobailHomeArticle(platFrom);
		} catch (Exception e) {
	...
	public Object findOperin() {		
		...
		lottvo.setPlatform(PlatFormEnum.ANDROID.getValue());
	...
	public Object findNewVersion(OperateLottSoftwareVersionVO vo){
		...
		criteria.setType(MobailPlatFormEnum.ANDROID.getValue());
