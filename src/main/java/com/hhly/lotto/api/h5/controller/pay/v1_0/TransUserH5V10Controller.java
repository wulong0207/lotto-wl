 ...
 */
@RestController
@RequestMapping("/h5/v1.0/trans")
public class TransUserH5V10Controller extends BaseController {

	private static final Logger logger = Logger.getLogger(TransUserH5V10Controller.class);
	...
	*/
	...
	public Object transList(HttpServletRequest request) {
		...
		try {
			logger.debug("请求参数：" + JSON.toJSONString(transParam));
			ResultBO<?> findAppTransUserByPage = transUserService.findAppTransUserByPage(transParam);
			logger.debug("查询交易详情返回：" + JSON.toJSONString(findAppTransUserByPage));
			return findAppTransUserByPage;
		} catch (Exception e) {
	...
	*/
	...
	public Object takenList(HttpServletRequest request) {
		...
		TransParamVO transParam = (TransParamVO) resultBO.getData();
		try {
			transParam.setPlatform(PayConstants.TakenPlatformEnum.WAP.getKey());
