 ...
 * @company 益彩网络科技公司
 */
public class OrderCommonV10Controller extends BaseController {

    private static Logger logger = LoggerFactory.getLogger(OrderCommonV10Controller.class);
    ...
    /**
	 * 活动服务:追号撤单时验证是否是活动的追号单，有些活动单禁止撤
	 */
    @Autowired
     ...
     * @return ResultBO<?>
     */
    @RequestMapping("queryYearOrderInfoDetail")
    public ResultBO<?> queryYearOrderInfoDetail(OrderQueryVo orderQueryVo){
    	Assert.notNull(orderQueryVo.getToken());
    	Assert.notNull(orderQueryVo.getActivityCode(),"活动不能为空");
    	return iorderSearchService.queryYearOrderInfoDetail(orderQueryVo);
    }
    ...
    public ResultBO<?> queryAddOrderListInfo(@RequestBody OrderQueryVo orderQueryVo){
        ...
        if(orderQueryVo.getAddQueryType()==Constants.NUM_2){
            orderQueryVo.setActivityCodes(Arrays.asList(orderQueryVo.getActivityCode().split(SymbolConstants.COMMA)));
        }
        try {
    ...
    public ResultBO<?> queryOrderDetailInfo(@RequestBody OrderSingleQueryVo orderSingleQueryVo){
    	logger.debug("订单详情-竞技彩和数字彩和老足彩和北京单场");
    ...
    public ResultBO<?> queryUserNumOrderList(@RequestBody UserNumOrderDetailQueryVO queryVO){
    	logger.debug("数字彩订单详情");
        if(ObjectUtil.isBlank(queryVO) || ObjectUtil.isBlank(queryVO.getOrderCode())
    ...
    public ResultBO<?> queryUserChaseWinningDetail(@RequestBody UserChaseDetailQueryVO queryVO){
    	logger.debug("追号计划订单详情的每期中奖明细列表");
        if(ObjectUtil.isBlank(queryVO) || ObjectUtil.isBlank(queryVO.getOrderAddCode()) || ObjectUtil.isBlank(queryVO.getToken())){
            return ResultBO.err(MessageCodeConstants.PARAM_IS_NULL_FIELD);
        }
        try {
    ...
    public ResultBO<?> queryOrderAmountDetail(@RequestBody OrderSingleQueryVo orderSingleQueryVo){
        ...
        try {
            ...
            if(orderSingleQueryVo.getType()==Constants.NUM_1){//购彩
                ...
                orderInfoSingleBO.setAddRedAmount(orderBaseInfoBO.getGetRedAmount());
            }
